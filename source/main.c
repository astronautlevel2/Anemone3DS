...
u8 themeInstall()
{
	...
	body = malloc((sizeof(char) * bodySize) + 8); // just in case
	FSFILE_Read(bodyHandle, 0, 0, body, (u32)bodySize);
	...
	bgm = malloc((sizeof(char) * bgmSize) + 8); // just in case
	FSFILE_Read(bgmHandle, 0, 0, bgm, (u32)bgmSize);
	...
	FSFILE_Read(saveDataHandle, 0, 0, saveDataBuffer, (u32)saveDataSize);

	memset(&saveDataBuffer[0x13b8], 0, 8); // I don't know what this does tbqh, taken straight from the docs
		
	saveDataBuffer[0x141b] = 0;
	saveDataBuffer[0x13bd] = 3;
	saveDataBuffer[0x13b8] = 0xFF;

	FSFILE_Write(saveDataHandle, 0, 0, saveDataBuffer, saveDataSize, FS_WRITE_FLUSH);
	...
	FSFILE_Write(bodyCache, 0, 0, &body[0], (u64)bodySize, FS_WRITE_FLUSH);
	...
	FSFILE_Write(bgmCache, 0, 0, &bgm[0], (u64)bgmSize, FS_WRITE_FLUSH);
	...
	FSFILE_Read(themeManageHandle, 0, 0, themeManageBuffer, (u32)2048);
	...
	*bgmSizeLocation = (u32)bgmSize;
	...
	FSFILE_Write(themeManageHandle, 0, 0, themeManageBuffer, 2048, FS_WRITE_FLUSH);
