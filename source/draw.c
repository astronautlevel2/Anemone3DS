...
void draw_install(InstallType type)
{
    ...
    switch(type)
    {
        ...
        case INSTALL_LOADING_SPLASHES:
            ...
            break;
        case INSTALL_LOADING_BADGES:
            pp2d_draw_text_center(GFX_TOP, 120, 0.8, 0.8, COLOR_WHITE, "Loading badges, please wait...");
            break;
...
void draw_interface(Entry_List_s* list, Instructions_s instructions)
{
    ...
    if(list->entries == NULL)
    {
        ...
        for(int i = 0; i < MODE_AMOUNT; i++)
            pp2d_draw_textf(320-(24*(i+1))+2.5, -3, 1, 1, COLOR_WHITE, "%c", *(mode_string[i]));
    ...
    if(list->scroll + list->entries_per_screen < list->entries_count)
        pp2d_draw_texture_flip(TEXTURE_ARROW, 155, 224, VERTICAL);

    for(int i = list->scroll; i < (list->entries_per_screen + list->scroll); i++)
    {
        ...
        int vertical_offset = list->entry_size * (i - list->scroll);
        u32 font_color = COLOR_WHITE;

        if(i == list->selected_entry)
        {
            font_color = COLOR_BLACK;
            pp2d_draw_rectangle(0, 24 + vertical_offset, 320, list->entry_size, COLOR_CURSOR);
        }
        pp2d_draw_wtext(list->entry_size+6, 40 + vertical_offset, 0.55, 0.55, font_color, name);
        if(!current_entry.placeholder_color)
        {
            ssize_t id = 0;
            if(list->entries_count > list->entries_per_screen*ICONS_OFFSET_AMOUNT)
            {
                id = list->icons_ids[ICONS_VISIBLE*list->entries_per_screen + (i - list->scroll)];
            }
            else
                id = list->icons_ids[i];
            pp2d_draw_texture_part(id, 0, 24 + vertical_offset, 0, 0, list->entry_size, list->entry_size);
        }
        else
            pp2d_draw_rectangle(0, 24 + vertical_offset, list->entry_size, list->entry_size, current_entry.placeholder_color);
